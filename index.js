...
function removeArtist(array,number) {
  const takeAway = [...array];

  takeAway.splice(number,1);
...
function addArtist(array, inpName, inpYears, inpGenre, inpNationality, inpBio) {
...
const byCountry = [];
  for(let i = 0; i < array.length; i++)
    {
          if(array[i].nationality === national)  
          {
            let trueArtist = array[i].name;
            byCountry.push(trueArtist);
            }
    }
  return byCountry;
